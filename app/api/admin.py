...
async def _ping_groq() -> bool:
    try:
        ...
        logger.debug("Groq health check failed: {}", exc)
        return False
...
async def health_check() -> dict[str, Any]:
    ...
    groq_ok = bool(results[3]) if not isinstance(results[3], Exception) else False
    tavily_ok = bool(results[4]) if not isinstance(results[4], Exception) else False

    all_ok = postgres_ok and qdrant_ok and redis_ok and groq_ok and tavily_ok
